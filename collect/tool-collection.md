...
仮説検証に必要な情報をピンポイントで収集する。網羅的に全データを取得する必要はない。
...
Bot Token のセットアップ手順は各ツールの公式ドキュメントを参照。
...
Integration Token のセットアップ手順は各ツールの公式ドキュメントを参照。
...
Access Token のセットアップ手順は各ツールの公式ドキュメントを参照。
...
収集したデータはスキル内の `data/` ディレクトリに保存し、次回実行時に再利用する。

...
上記は代表例であり、**これ以外のMCPツールが `permitted_sources` に含まれている場合も同様に収集する**（後述の「その他のMCPツール収集手順」を参照）。
...
上記に列挙されていないMCPツールが `permitted_sources` に含まれている場合、以下の方針で収集する。

```
① そのMCPツールで利用可能なアクション（list系・get系）を確認
② 一覧取得系のアクションで業務に関連するデータの概要を取得
③ ツールの種類に応じて収集内容を判断:
   - ワークフロー自動化ツール（n8n, Zapier等）: ワークフロー一覧・状態・何を自動化しているかの要約
   - プロジェクト管理ツール（Wrike, Asana等）: タスク一覧・ステータス・担当者・頻度パターン
   - その他: 業務パターンの推定に役立つ概要情報

収集する情報: 名前・状態・業務との関連の要約
保存しない情報: APIキー・認証情報・具体的なURL/ID・個人情報
```

ワークフロー自動化ツールの情報が得られた場合は `existing_automation` フィールドとして出力JSONに含める。これにより分析エージェントが🔧（新規仕組み化）と🔩（既存仕組み改善）を正確に分類できる。
...
```json
{
  ...
  "existing_automation": [
    { "name": "ワークフロー名", "status": "active | inactive", "summary": "何を自動化しているかの要約", "source": "ツール名" }
  ]
}
```

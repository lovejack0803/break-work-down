...
- **接続済みツールからの収集許可**: Step 1a で検出した接続済みMCPツールを選択肢に並べ、どれを収集対象にするか聞く。「収集しない」も選択肢に含める。ツールの種類は問わない（コミュニケーション: Slack, Discord / ナレッジ管理: Notion / グループウェア: Google Workspace / プロジェクト管理: GitHub, Wrike 等）。

...
ゴールは「読み手がまず全体像を掴み、次に各提案の詳細を理解し、最後に次の一歩を決められる」こと。全業務を網羅的に並べるのではなく、1位の提案に深掘りする構成にする。
...
**個人名・会社名の扱い:** レポート内では会社名・個人名を使用しない。部署名・役職名・業務内容の記載は問題ない。カバーのメタ情報やタイトルにも会社名は含めず、部署名＋日付で構成する。
